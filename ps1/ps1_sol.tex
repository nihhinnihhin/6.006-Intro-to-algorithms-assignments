...
1, 2, 4, 3(takes the logarithm and limitation.)
...
First, we formalize the algorithm as below:


\textbf{\texttt{Algorithm4}: }

\begin{itemize}
	\item Pick middle column $ j=m/2 $ or row $ i=n/2 $.
	\item Find global maximum on column $ j $ or on row $ i $ at $ (i,j) $.
	\item Compare $ (i,j-1) $, $ (i,j) $, $ (i,j+1) $ or similarly for row method.
	\item Pick left columns of $ (i,j-1) > (i,j) $ or similarly for row method.
	\item Similarly for right or similarly for row method.
	\item $ (i,j) $ is a 2D-peak if neither condition holds. 
	\item If the previous steps is via column, then solve the new problem with half the number of columns through row, otherwise column.
	\item When you have a single column or row, find global maximum and you're done.
\end{itemize}
...
\begin{quote}
	...
	$m \times \lfloor n / 2 \rfloor$ or 
	$m \times \left(n - \lfloor n / 2 \rfloor - 1 \right)$.
	or similarly for the row version: 
	$ \left[ m/2 \right]  \times n $ or 
	$ \left( m - \left[ m/2 \right] -1 \right) \times n$.	
	Therefore, the number of columns or the rows in the problem
	strictly decreases with each recursive call
	as long as $n \text{ or } m > 0$.
	...
	Similarly for the case that the problem ends with size $ 1 \times n $ or $ 2 \times n $.
