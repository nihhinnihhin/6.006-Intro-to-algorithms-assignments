# Error information
# =============================================================================
# A:\001SimulationLab\pylab\6.006\ps2\circuit>python circuit.py < tests/1gate.in > out
# Traceback (most recent call last):
#   File "circuit.py", line 632, in <module>
#     sim.run()
#   File "circuit.py", line 514, in run
#     in_transition[0]))
#   File "circuit.py", line 398, in append
#     self.increase((len(self.queue)-1),key)
#   File "circuit.py", line 385, in increase
#     if key > self.queue[i]:
#   File "circuit.py", line 298, in __gt__
...
class PriorityQueue:
    ...
    query: HeapMax(A)
    update: maxHeapify(A,i), build(A), heapsort(A), insert(A,key), extract(A),increase(A,i,key),
    ...
    def maxHeapify(self,i):
        L=self.left(i)
        R=self.right(i)
        if L<=self.length and self.queue[L] >self.queue[i]:
            largest=L
        else: largest=i
        if R<=self.length and self.queue[R] >self.queue[i]:
            largest=R
        if largest != i:
            temp=self.queue[i]
    ...
    def increase(self,i,key):
        ...
        self.queue[i]=key
        while i>1 and self.queue[i] < self.queue[self.parent(i)]:
            temp=self.queue[i]
    ...
    def append(self, key):
        ...
        self.queue.append(float('inf')) # Equivalent to math.inf (new in version 3.x)
        self.length +=1
        self.increase((self.length-1),key)
