...
#            print 'count',count
    ...
    #        print nbMovePairsB,nbMovePairsE
            for i in range(6):
                vB=nbMovePairsB[i][NEIGHBOR]
                vE=nbMovePairsE[i][NEIGHBOR]
                
                if vB not in rB.level:
                    if newB:prevB=newB  # prevB is the last new found, so it won't be None
...
#                    if currLevel==2:
#                        print prevB
#                    if vB is the first element in the level, then create a new set 
...
#                    if vE is the first element in the level, then create a new set
