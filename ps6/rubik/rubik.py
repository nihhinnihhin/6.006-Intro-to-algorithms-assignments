...
def perm_to_string(p):
    ...
    """
    s = "("
    for x in p: s = s + "%2d "%x    # %x probably means 'x'
    s += ")"
    return s
...
To visualizely see the patternof in the Front face rotated clockwise case 
that all elements move rightward three element-unit, 
click the element entrywise and see the same word highlighted.
"""
...
def twistNameInverse(move):
    """
    return the inverse move name of given the move name, 
    e.g. twistNameInverse('Fi')='F'
    """
    if  move=='F':
        return 'Fi'
    elif move=='Fi':
