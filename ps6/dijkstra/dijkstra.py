...
# Note: reimplement the __eq__ and __ne__ of class Node and NodeDistancePair 

# node and nodeDistPair instances are unhashable
...
class Network(object):
    ...
    def verify_path(self, path, source, destination):
        ...
        for i in range(len(path) - 1):
            if (path[i].featureid, path[i+1].featureid) not in self.edge_set and \
                (path[i+1].featureid, path[i].featureid) not in self.edge_set:
    ...
    def _create_adjacency_lists(self):
        ...
        for node in self.nodes:
            node.adj = []   
            # ???unlike C++, attribute could be added to an object any time
    ...
    def _create_edge_set(self):
        ...
        edge_set = set()
        for link in self.links:
            edge_set.add((link.begin.featureid, link.end.featureid))
        return edge_set
...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        Q=PriorityQueue()
        S=set()
        id_pair={} # map node featureid to NodeDistancePair(NDP), 
       
        # create the unique NodeDistPair instance for every node, 
        # then all identifiers would just be pointer.
        # Note: adj's elements are still Node, so we use id_pair to map Node indectly to NDP(through featureid)
        for node in nodes:
            if node==source:    
#                print 'node type is Node: {0}, destination type is Node: {1}'\
#                .format(node==Node(1,2,3,4,5),source==NodeDistancePair(Node(1,2,3,4,5),0))
                source=NodeDistancePair(node,0)
                Q.insert(source)
                id_pair[node.featureid]=source
            elif node==destination:
                destination=NodeDistancePair(node,float('inf'))
                Q.insert(destination)
                id_pair[node.featureid]=destination
            else:
                nodeDistPair=NodeDistancePair(node,float('inf'))
                Q.insert(nodeDistPair)
                id_pair[node.featureid]=nodeDistPair
        # relax
        parents={source:None}
        while Q:
            u=Q.extract_min()
            ...
            for v in u.node.adj:    # u: NodeDistancePair; v: Node
#                print 'u {0}, v {1}'.format(u,v)
                vTemp=id_pair[v.featureid]  # vTemp: NodeDistancePair
                if vTemp.distance > u.distance+weight(u.node,vTemp.node):
                    vTemp.distance=u.distance+weight(u.node,vTemp.node)
                    Q.decrease_key(vTemp)
                    parents[vTemp]=u
        # results:            
        SP=[]   # list of Node instance, not NDP
        parent=u
        number=len(parents)
        while(parent):
            SP.append(parent.node)
