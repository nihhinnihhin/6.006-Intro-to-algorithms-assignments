...
2 since \#\text{perchased stocks} $\le$ 3, if 2, 

then 2 Pear's will defeat 2 Dale's.
%%% PROBLEM 2(b) PEAR ANSWER END %%%
}

\newcommand{\answerIIcDale}{
%%% PROBLEM 2(c) DALE ANSWER START %%%
10, since the rate, earn over cost, of Dale is the largest.
...
3 5
\begin{enumerate}
	\item No, since in DP, we care about the $ state $ $ transition $ determined by $ cost $ and $ profit $, etc. not $ time delay $, we can ignore this factor.
	\item No, obvious.
	\item Yes, in $ max $ function, it contributes.
	\item No, the $ price $ corresponds to $ size $
	\item Yes, in the lecture's Knapsack problem, we just considered packing the stuff or not, not how many of each one.
	\item same as reason 1, the $ profit $ influences the $ state $, which have nothing to do with selling items or not.
\end{enumerate} 
...
10. $\Theta\left(count\cdot total\right)$
...
{\color{red} 
	CORRECT: 1. $\Theta\left(count\right)$
	
	WRONG: 10. $\Theta\left(count\cdot total\right)$ }
...
{\color{red} 
	CORRECT: 7. $\Theta\left(count+total\right)$
